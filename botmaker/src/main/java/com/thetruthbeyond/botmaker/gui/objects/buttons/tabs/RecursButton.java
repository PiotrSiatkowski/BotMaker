...
import com.thetruthbeyond.botmaker.gui.tabs.teachtab.RecursTab;
...
public class RecursButton extends TabButton {
	
	public RecursButton(FileManager loader, Clickable parent) {
		super(parent);
		initialize(loader);
	}
	
	public RecursButton(Area area, FileManager loader, Clickable parent) {
		super(area, parent);
		initialize(loader);
	}

	protected void initialize(FileManager loader) {
		button = loader.getTexture("Recursion");
		buttonHover = loader.getTexture("RecursionHover");
		shape = Shape.Oval;
