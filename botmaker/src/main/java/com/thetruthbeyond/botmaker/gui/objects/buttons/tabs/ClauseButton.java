...
import com.thetruthbeyond.botmaker.gui.tabs.teachtab.ClauseTab;
...
public class ClauseButton extends TabButton {
	
	public ClauseButton(FileManager loader, Clickable parent) {
		super(parent);
		initialize(loader);
	}
	
	public ClauseButton(Area area, FileManager loader, Clickable parent) {
		super(area, parent);
		initialize(loader);
	}

	protected void initialize(FileManager loader) {
		button = loader.getTexture("Condition");
		buttonHover = loader.getTexture("ConditionHover");
		shape = Shape.Oval;
