...
public class CapitalCaseTextField extends TextFieldDecorator {
	
	public CapitalCaseTextField(TextField parent) {
		super(parent);
	}
	...
	private void makeCapitalCase() {
		String input = getInput();
		if(input.length() == 1) {
			input = input.toUpperCase();
			super.setInput(input);
		} else if(input.length() > 1) {
			input = input.substring(0, 1) + input.substring(1, input.length()).toLowerCase();
			super.setInput(input);
		}
	}
}